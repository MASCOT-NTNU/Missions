...
plt.scatter(y1, x1, c = sal1, cmap = 'viridis')
plt.title("Temperature variation along transect lines")
plt.colorbar()
# plt.savefig(figpath + "Sal1.pdf")
plt.show()

#%%
plt.plot(sal1)
# plt.savefig(figpath + "Sal1_side.pdf")
plt.show()
...
# V_v = Variogram(coordinates = np.array([x1, y1]).T, values = sal1)
# print(V_v)
...
sigma = 85  # scaling coef in matern kernel
...
mu_post = mu_prior + Sigma_loc_obs @ np.linalg.solve(Sigma_obs, sal1)

mu_post = mu_post.reshape(n1, n2)
fig = plotf(mu_post, "this is trondheim fjord temperature", xlim = XLIM, ylim = YLIM)
fig.savefig(figpath + "Temp_post.pdf")
...
BBox = ((lon_all.min(), lon_all.max(),lat_all.min(), lat_all.max()))
