...
# distance = 100
distance = 1000
depth_obs = 2.0  # planned depth to be observed
...
if depth_obs / 0.5 % 2 == 0:
    odd = False
    F_sampling = F_even
    print("Now it is even layer")
else:
    odd = True
    F_sampling = F_odd
    print("Now it is odd layer")
...
def save_data(datapath, timestamp, data_lat, data_lon, data_x, data_y, data_z, data_salinity, data_temperature):
    ...
    np.savetxt(datapath + "data_{:02d}.txt".format(No_Script), data, delimiter=",")

logfile = open(datapath + "log_{:02d}.txt".format(No_Script), "w+")
